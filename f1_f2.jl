...
usecase = "ai50" # "smartcity", "iiot", "ai", "vr"

# Paths
service_file = "services/types/" * usecase * ".csv"
results_path = "results/usecase/" * usecase * "/f1_f2/"
...
for gap in mip_gaps
    ...
    creation_time = @elapsed model, f1, f2 = create_model(DC, I, D, T, l, CO, CR, CS, C, M, G, f, c, m, g, lat_threshold, R, Ir, Io, Is, Dt, N, duration, gap)
	...
	objectives = [(latency[i], cost[i]) for i in eachindex(latency)]
