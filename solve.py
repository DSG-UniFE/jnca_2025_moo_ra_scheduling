from multi_cluster_cnsm import MultiCluster
...
if __name__ == '__main__':
    ...
    algorithm = NSGAIII(
        ...
        termination_criterion=StoppingByEvaluations(max_evaluations=25_000),
    ...
    #plot_front.plot(front, label='NSGAIII-MultiCluster-Problem', filename='NSGAIII-MultiCluster', format='png')
    ...
    print(f'Total non-dominated solutions plotted: {len(front)}')
    plot_front = Plot(title='Pareto front approximation', axis_labels=['f1', 'f2', 'f3'])
    #plot_front.plot(front, label='NSGAIII-MultiCluster-Problem', filename='NSGAIII-MultiCluster', format='png')
    plot_front.plot(front, label=f"{algorithm.get_name()}-MultiCluster-Problem", filename=f"{algorithm.get_name()}-MultiCluster4", format='png')
    # let's get the front for f1-f2, f1-f3, f2-f3
