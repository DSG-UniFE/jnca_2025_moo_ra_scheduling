from moo_ra_f1f3 import MooRaF1F3
...
def logplot_front(front, algorithm, output_dir, problem_name):
    ...
    plot_front.plot(front, label=f"{algorithm.get_name()}-f1-f3", filename=f"{front_filename}", format='png')
...
if __name__ == '__main__':
    ...
    for service_file in services_files:
        ...
        result_dir = os.path.join("results_f1f3", base_name)
        ...
        for algorithm in algorithms:
            ...
            for s in front:
                ...
                print(
                    f"F1: {s.objectives[0]}, F3: {s.objectives[1]}, Constraints: {s.constraints}"
                )
