...
def plot_combined_2d_front(meta_solutions_dict, ilp_solutions_dict, save_path):
    ...
    legend_meta = ax.legend(handles_meta, labels_meta, 
                             loc="upper left", bbox_to_anchor=(0.75, 0.87),
    ...
    # Set axis labels
    ax.set_xlabel("Deployment Costs (Obj 2)", labelpad=10, fontdict={"fontsize": 14})
    ax.set_ylabel("Avg. Interruption Frequency (Obj 3)", labelpad=10, fontdict={"fontsize": 14})
...
def main():
    ...
    for usecase in dir_usecases_ilp:
        ...
        os.makedirs(output_dir, exist_ok=True)
        objectives_files_ilp = glob.glob(f"{usecase}f2_f3/results*_mac.csv")
    ...
    for usecase in dir_usecases_meta:
        ...
        objectives_files_meta = glob.glob(f"results_f2f3/{usecase}/*.FUN.*")
        ...
        combined_plot_path = os.path.join(result_dir,  f"combined_plot_{usecase_name}_f2f3.pdf")
