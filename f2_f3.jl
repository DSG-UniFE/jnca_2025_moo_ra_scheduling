...
usecase = "iiot50" # "smartcity", "iiot", "ai", "vr"

# Paths
service_file = "services/types/" * usecase * ".csv"
results_path = "results/usecase/" * usecase * "/f2_f3/"
...
for gap in mip_gaps
    ...
    creation_time = @elapsed model, f2, f3 = create_model(DC, I, D, T, l, CO, CR, CS, C, M, G, f, c, m, g, lat_threshold, R, Ir, Io, Is, Dt, N, duration, gap)
	...
	objectives = [(cost[i], unavailability[i]) for i in eachindex(cost)]
	...
	pareto_cost = [sol[1] for sol in pareto_solutions]
	...
	Plots.scatter!(
		pareto_cost, pareto_unavailability;
		color = :red,
		markersize = 7,
		label = "Pareto Front"
	)
